vcl 4.0;
########## File located at /etc/varnish/default.vcl ##########
...
sub vcl_recv {
  ...
  # The admin interface is only allowed to the IP belonging to the admin list.
  ...
  if (req.http.Cache-Control ~ "(?i)no-cache" && client.ip ~ editors) { # create the acl editors if you want to restrict the Ctrl-F5
    ...
    if (! (req.http.User-Agent ~ "(?i)bot" || req.http.X-Purge)) {
...
sub common_pass {
  ...
  if (req.url ~ "^/(admin|user)" ||
      req.url ~ "^/(install|update|cron)\.php") {
    return (pass);
  }
}
...
sub vcl_hit {
  ...
  # There are two potential problems - one is a thundering herd problem - suddenly releasing a thousand threads to serve content might send the load sky high.
  # Secondly - nobody likes to wait. To deal with this we can instruct Varnish to keep the objects in cache beyond their TTL and to serve the waiting requests somewhat stale content.
